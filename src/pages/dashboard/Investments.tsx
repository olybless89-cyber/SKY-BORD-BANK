  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {PLANS.map((plan) => (
          <div key={plan.name} className={cn('rounded-2xl p-8 border transition-all', plan.featured ? 'bg-gradient-to-br from-primary to-[#0a1c50] border-primary teal-glow' : 'glass-card border-border')}>
            ...
            {investments.map((inv) => (
              <div key={inv.id} className="flex items-center gap-4 p-4">
                ...
                <div className="text-right shrink-0">
                  ...
                  <Badge className={cn('text-xs border-0', inv.status === 'active' ? 'bg-green-500/10 text-green-600' : 'bg-muted text-muted-foreground')}>{inv.status}</Badge>
                </div>
              </div>
            ))}
          </div>
