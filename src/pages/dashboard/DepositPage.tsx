...
import { getUserAccounts, submitDepositRequest, getUserDepositRequests } from '@/services/api';
...
import type { Account, DepositRequest } from '@/types/types';
...
export default function DepositPage() {
  ...
  const reload = () => {
    if (!user) return;
    Promise.all([getUserAccounts(user.id), getUserDepositRequests(user.id)])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const { error } = await submitDepositRequest(user!.id, accountId, amt);
    setLoading(false);
    if (error) { toast.error('Failed to submit request: ' + error.message); return; }
  ...
  return (
    <DashboardLayout>
      <div className="p-6 space-y-6">
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Deposit Funds</h1>
          <p className="text-muted-foreground text-sm mt-1">Submit a deposit request for admin approval</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>Your request will be reviewed and credited by an admin</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <CardContent className="p-0">
              {requests.length === 0 ? (
                <div className="text-center py-10 px-6">
                  <ArrowDownLeft className="h-8 w-8 text-muted-foreground mx-auto mb-3 opacity-40" />
