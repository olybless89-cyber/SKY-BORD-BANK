...
const TYPE_COLORS: Record<string, string> = { deposit: 'text-green-400', withdrawal: 'text-red-400', transfer: 'text-primary', interest: 'text-yellow-400' };
const STATUS_COLORS: Record<string, string> = { completed: 'bg-green-500/10 text-green-400', pending: 'bg-yellow-500/10 text-yellow-400', failed: 'bg-red-500/10 text-red-400', cancelled: 'bg-muted text-muted-foreground' };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="relative">
        ...
        <Input placeholder="Search by description or reference..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-secondary border-border" />
      </div>
                  ...
                  {filtered.map((txn) => {
                    ...
                    return (
                      <tr key={txn.id} className="hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <span className={cn('font-semibold text-sm', isCredit ? 'text-green-400' : 'text-red-400')}>
